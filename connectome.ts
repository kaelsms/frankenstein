// ═══════════════════════════════════════════════════════════════
// C. elegans connectome (subset) — real neuron names, real connections
// Based on White et al. 1986, Varshney et al. 2011, Cook et al. 2019
// ═══════════════════════════════════════════════════════════════
...
export interface Neuron {
  ...
  refractory: number // ms of refractory period
  label?: string
}
...
export interface Synapse {
  ...
  type: 'chemical' | 'electrical' | 'bridge'
  delay: number // ms
}

// ═══ C. elegans neurons (sensory-motor circuit) ═══
// Key neurons from the locomotion and chemotaxis circuits
const WORM_NEURONS: Omit<Neuron, 'potential' | 'lastFired'>[] = [
  // Sensory neurons
  ...
  // Command interneurons (forward/backward)
...
// ═══ Drosophila mushroom body (simplified) ═══
const FLY_NEURONS: Omit<Neuron, 'potential' | 'lastFired'>[] = [
  // Kenyon cells (associative memory)
  { id: 'KC1', type: 'inter', system: 'fly', x: 0.15, y: 0.2, threshold: -52, refractory: 4, label: 'Kenyon cell α' },
  { id: 'KC2', type: 'inter', system: 'fly', x: 0.15, y: 0.35, threshold: -52, refractory: 4, label: 'Kenyon cell β' },
  { id: 'KC3', type: 'inter', system: 'fly', x: 0.15, y: 0.5, threshold: -52, refractory: 4, label: 'Kenyon cell γ' },
  { id: 'KC4', type: 'inter', system: 'fly', x: 0.15, y: 0.65, threshold: -52, refractory: 4, label: "Kenyon cell α'" },
  { id: 'KC5', type: 'inter', system: 'fly', x: 0.15, y: 0.8, threshold: -52, refractory: 4, label: "Kenyon cell β'" },

  // Mushroom body output neurons (MBON)
  { id: 'MBON1', type: 'inter', system: 'fly', x: 0.4, y: 0.15, threshold: -48, refractory: 5, label: 'MBON-α2' },
  { id: 'MBON2', type: 'inter', system: 'fly', x: 0.4, y: 0.35, threshold: -48, refractory: 5, label: 'MBON-β2' },
  { id: 'MBON3', type: 'inter', system: 'fly', x: 0.4, y: 0.55, threshold: -48, refractory: 5, label: 'MBON-γ5' },
  { id: 'MBON4', type: 'inter', system: 'fly', x: 0.4, y: 0.75, threshold: -48, refractory: 5, label: 'MBON-calyx' },

  // Dopaminergic neurons (reward/punishment)
  { id: 'DAN1', type: 'modulatory', system: 'fly', x: 0.6, y: 0.25, threshold: -50, refractory: 8, label: 'DAN PPL1-α3' },
  { id: 'DAN2', type: 'modulatory', system: 'fly', x: 0.6, y: 0.5, threshold: -50, refractory: 8, label: 'DAN PAM-β2' },
  { id: 'DAN3', type: 'modulatory', system: 'fly', x: 0.6, y: 0.75, threshold: -50, refractory: 8, label: 'DAN PPL1-γ2' },
...
// ═══ Bridge neurons (novel — the hybrid part) ═══
const BRIDGE_NEURONS: Omit<Neuron, 'potential' | 'lastFired'>[] = [
...
// ═══ Synapses ═══
// C. elegans connections (from Varshney et al. 2011 adjacency data)
const WORM_SYNAPSES: Omit<Synapse, 'delay'>[] = [
  // Sensory → interneuron
...
// Fly connections
const FLY_SYNAPSES: Omit<Synapse, 'delay'>[] = [
  // KC → MBON (learned associations)
...
// Bridge connections (the novel part — cross-species wiring)
const BRIDGE_SYNAPSES: Omit<Synapse, 'delay'>[] = [
  // Worm sensory → bridge
  ...
  // Fly decisions → bridge → worm motor
  ...
  // Fly reward → bridge → worm modulation
...
export function buildConnectome() {
  ...
  const init = (n: Omit<Neuron, 'potential' | 'lastFired'>): Neuron => ({
    ...n, potential: -70, lastFired: -1000,
  })
  ...
  const addSynapses = (list: Omit<Synapse, 'delay'>[], defaultDelay: number) => {
    for (const s of list) {
